...
return new class extends Migration
{
    ...
    public function up()
    {
        ...
        DB::unprepared('CREATE OR REPLACE VIEW data_rombel AS SELECT r.id ,r.SMP ,k.nama_kelas, t.NIG, t.name, th.TahunAjaran,th.status,r.id_thnakademik, jumlah_siswa_kelas(r.id,r.id_thnakademik) as jumlah FROM rombels r INNER JOIN kelas k ON k.id=r.id_kelas INNER JOIN teachers t ON t.NIG=r.id_wali INNER JOIN tahun_akademiks th ON th.id=r.id_thnakademik');

        //menampilkan daftar siswa di kelas/rombel  VIEW #4
        DB::unprepared('CREATE OR REPLACE VIEW data_rombel_siswa AS SELECT rs.id_rombel ,s.name ,rs.id_siswa, s.NIS ,rs.id, r.id_thnakademik ,th.status FROM rombel_siswas rs INNER JOIN students s ON s.NIS=rs.id_siswa INNER JOIN rombels r ON r.id=rs.id_rombel INNER JOIN tahun_akademiks th ON th.id=r.id_thnakademik');
